...
par(mfrow = c(2,2))                   # set a matrix of plot of dimension 2 by 2
...
plot(                                                            # create a plot
    ...
    ylab = 'Global Active Power'            # and a label for y axis
)

# topright
plot(                                                            # create a plot
    x = data$Time,                             # of time as indipendent variable
    y = data$V,               # and Global Active Power (Gap) as dipendent one
    type = 'l',                                                # do a lined plot
    xlab='datetime',                                       # additional x labels
    ylab = 'Voltage'            # and a label for y axis
)
...
legend(                                                         # add the legend
    ...
    box.lwd = 0,                                  # this time without the border
...
plot(                                                            # create a plot
    x = data$Time,                             # of time as indipendent variable
    y = data$Grp,               # and Global Active Power (Gap) as dipendent one
    type = 'l',                                                # do a lined plot
    xlab='datetime',                                       # additional x labels
    ylab = 'Global_reactive_power' # and label for y axis (with '_')
)
